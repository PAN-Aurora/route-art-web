...
/**
 * Description：资产管理-退役报废计划Controller层
 *
 * @author： 陈磊     Date：2020-12-17
 */
@Controller
@RequestMapping("/tybf")
public class TybfController extends BaseController {
    /**
     * 注入资产管理-退役报废计划的service层
     */
    @Autowired
    private TybfService tybfService;

    /**
     * 注入资产管理-退役报废明细的service层
     */
    @Autowired
     ...
     *
     * @param
     * @return map 资产管理-退役报废计划的数据
     * @throws Exception
     */
    @RequestMapping(value = "/add")
    @RequiresPermissions("tybf:add")
    @ResponseBody
    ...
    public Object delete() throws Exception {
        ...
        pd = this.getPageData();
        //资产管理-退役报废计划删除方法
        tybfService.delete(pd);
        //关联删除明细数据
        tybfmxService.deleteByXljhID(pd);
        //返回结果
        map.put("result", errInfo);
        return map;
    }
    ...
    public Object tjsave(Page page) throws Exception {
        ...
        try {
            ...
            //用于获取明细信息
            map.put("JHID", pd.getString("TYBF_ID"));
            //设置提交用户为当前用户的姓名
            map.put("TJYH", Jurisdiction.getName());
            //指派代理人为当前用户
            map.put("USERNAME", Jurisdiction.getUsername());
            //指定跳转路径
            map.put("tzUrl", Const.TYBFVIEW);
            //记录存入数据库
            tybfService.tjsave(pd, map);
            //用于给待办人发送新任务消息
        ...
        rutaskPd.put("JHID", pd.getString("TYBF_ID"));
    ...
    public Object list(Page page) throws Exception {
        ...
        page.setPd(pd);
        //列出数据列表
        List<PageData> varList = tybfService.list(page);
        //将数据 page返回到前台
    ...
    public Object deleteAll() throws Exception {
        ...
        if (Tools.notEmpty(dataIds)) {
            ...
            //删除选择数据
            tybfService.deleteAll(arrDataIds);
            //删除成功，返回success
            errInfo = "success";
        } else {
            //没选择数据,返回提示信息
    ...
    /**
     * 导出资产管理-退役报废计划数据到excel
     *
     * @param
     * @return ModelAndView 资产管理-退役报废计划导出的数据
     * @throws Exception
     */
    ...
    public ModelAndView exportExcel() throws Exception {
        ...
        try {
            ...
            //查询要导出的数据集
            List<PageData> varOList = tybfService.listAll(pd);
            //创建ArrayList用来存放数据集
            ...
            for (int i = 0; i < varOList.size(); i++) {
                ...
                vpd.put("var1", varOList.get(i).getString("SQDH"));
                vpd.put("var2", varOList.get(i).getString("SSBM"));
                vpd.put("var3", varOList.get(i).getString("SQR"));
                vpd.put("var4", varOList.get(i).get("SQRQ").toString());
                vpd.put("var5", varOList.get(i).getString("BFYY"));
                vpd.put("var6", zt);
                //遍历数据放入集合中
                varList.add(vpd);
            }
    ...
    public Object handle() throws Exception {
        ...
        if ("yes".equals(msg)) {
            ...
            //查询退役报废数据
            List<PageData> lstData = tybfmxService.listAll(pd);

            //添加物资交接日志记录
            zcglLogService.saveData("TYBF", pd, lstData);
    ...
    public void exportWord(HttpServletResponse response, HttpServletRequest request) throws Exception {
        ...
        //导出word模板名称
        String strFilename = "退役报废.doc";
        // 导出数据集
        Map dataMap = new LinkedMap();
        // 构造退役报废基本信息
        dataMap = tybfService.findById(pd);

        // 构造明细信息
        List<Map> lstMxData = tybfmxService.listAllForWord(pd);
        // 添加到数据集中
        dataMap.put("tybfmx", lstMxData);
        //生成文件
        Freemarker.printFile("docTybf.ftl", dataMap, strFilename, Const.CON_WORD_TEMP_PATH, ftlPath);
