...
var vm = new Vue({
	...
	methods: {
	     ...
	     login: function() {
            ...
            $.ajax({
					...
					success: function(data){
                        if ("licenseError" == data.result) {
                            $(".verifyMsg span").css("display","block");
                            $(".verifyMsg span").text("您的证书无效，请检查服务器是否取得授权或重新申请证书");
                        }else
        ...
        init(){
            ...
            if('1' == msg){
                message('warning', '此用户在其它终端已经早于您登录,您暂时无法登录!', 1000);
            }else if('2' == msg){
                message('warning', '您被系统管理员强制下线或您的帐号在别处登录!', 1000);
