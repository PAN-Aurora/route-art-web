...
var vm = new Vue({
    ...
    methods: {
        ...
        init() {
            ...
            this.xmxx_id = getUrlKey('xmxx_id');
            this.getData();
        },

        //根据主键ID获取数据
        getData: function (xmxx_id) {
            //发送 post 请求
            $.ajax({
                xhrFields: {
                ...
                type: "POST",
                url: httpurl + 'ywpt/rjml/listRjTree?xmxx_id='+xmxx_id,
                data: { xmxx_id: xmxx_id, tm:new Date().getTime() },
                dataType: "json",
        ...
        savePublicRj() {
            if (this.$refs.tree.getCheckedNodes().length <= 0) {
                ...
                sessionStorage.setItem("ourj", JSON.stringify(jsonObject));
            }
            top.Dialog.close();
        },
    },
