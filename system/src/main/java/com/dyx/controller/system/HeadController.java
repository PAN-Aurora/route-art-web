...
public class HeadController extends BaseController {
	...
	public Object saveSysSet() throws Exception{
		...
		pd.put("tokenKey", Tools.creatTokenKey("sysSet"));

		//LoadBalancerUtil.responseByPost(this.loadBalancerClient, "fh-im", "main/saveSysSet", pd);	//同步到im微服务配置文件
	 ...
	 * @return
	 */
	@RequestMapping(value="/listMshostTodruid")
	@ResponseBody
	public Object listMshostTodruid()throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		String errInfo = "success";
		List<Dictionaries> dictList = dictionariesService.listSubDictByParentId("msid");	//msid 此ID为数字字典 微服务的ID，取其子一级数据
		for(Dictionaries dict : dictList){
			dict.setTreeurl("druid.html?mshost="+dict.getNAME_EN());
			dict.setSubDict(null);
			dict.setTarget("treeFrame");
		}
		JSONArray arr = JSONArray.fromObject(dictList);
	 ...
	 * @return
	 */
	@RequestMapping(value="/listMshostDb")
	@ResponseBody
	public Object listMshostDb(String type)throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		String errInfo = "success";
		List<Dictionaries> list	= new ArrayList<Dictionaries>();
		List<Dictionaries> dictList = dictionariesService.listSubDictByParentId("msid");					//msid 此ID为数字字典 微服务的ID，取其子一级数据
		for(Dictionaries dict : dictList){
			if("fh-dbsync".equals(dict.getNAME_EN()) || "fh-dbmanage".equals(dict.getNAME_EN())) {
				continue;//数控管理服务和表同步服务排除掉
			}
			dict.setTreeurl("index.html?mshost="+dict.getNAME_EN()+"&type="+type);
			dict.setSubDict(null);
			dict.setTarget("treeFrame");
			list.add(dict);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value="/listEureka")
	@ResponseBody
	public Object listEureka()throws Exception{
		Map<String,String> map = new HashMap<String,String>();
		String errInfo = "success";
		List<Dictionaries> list	= new ArrayList<Dictionaries>();
		List<Dictionaries> dictList = dictionariesService.listSubDictByParentId("eureka");					//eureka 此ID为数字字典 注册中心的ID，取其子一级数据
		for(Dictionaries dict : dictList){
			dict.setTreeurl("eureka.html?eurekanum="+(Integer.parseInt(dict.getORDER_BY())-1));
			dict.setSubDict(null);
			dict.setTarget("treeFrame");
			list.add(dict);
		}
